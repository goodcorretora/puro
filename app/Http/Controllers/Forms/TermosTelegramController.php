<?php

namespace App\Http\Controllers\Forms;

use App\Models\TermosTelegram;
use Illuminate\Http\RedirectResponse;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Auth;
use Illuminate\Support\Facades\Mail;
use App\Http\Controllers\Controller;

class TermosTelegramController extends Controller
{
    public function store(Request $request): RedirectResponse
    {
        $validated = $request->validate([
            'name' => 'required',
            'email' => 'required|email',
            'phone_celular' => 'required',
            'cpf' => 'required',
        ]);

        // Store data in database
        TermosTelegram::create([
            'user_id' => Auth::id(),
            ...$validated,
        ]);

        // Send mail to admin
        Mail::send('emails.telegram', $validated, function ($message) {
            $message->to('contato@goodcorretora.com.br', 'Solicitar Informações')
                    ->to('goodcorretora@gmail.com', 'Solicitar Informações')
                    ->subject($validated['name'] ?? 'Novo contato');
        });

        return redirect()->back()->with('success', 'Recebemos sua mensagem. Obrigado por entrar em contato!');
    }
}

